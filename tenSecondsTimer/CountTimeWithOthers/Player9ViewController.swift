...
class Player9ViewController: UIViewController,UITextFieldDelegate {
...
//    var playerNumber:Int?
    ...
    func goNext(playerNumber:Int){
        if isSaved!{
            ...
            let nextView = storyBoard.instantiateViewController(withIdentifier: "Player10ViewController") as! Player4ViewController
            nextView.name = "player10"
            self.present(nextView, animated: true, completion: nil)
            
        }else {
            let nav = UIAlertController(title: "十人目の名前を入力してね", message:nil, preferredStyle: .alert)
            ...
            nav.addAction(UIAlertAction(title: "次へ", style: UIAlertAction.Style.default, handler: {(action) in
                let storyBoard = UIStoryboard(name: "Main", bundle: nil)
                let nextView = storyBoard.instantiateViewController(withIdentifier: "Player10ViewController") as! Player10ViewController
