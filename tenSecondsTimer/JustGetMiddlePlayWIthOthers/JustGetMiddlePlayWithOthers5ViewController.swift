//
//  JustGetMiddlePlayWithOthers5ViewController.swift
//  tenSecondsTimer
//
...
class JustGetMiddlePlayWithOthers5ViewController: UIViewController,UITextFieldDelegate {
...
//        self.startStop.addTarget(self, action: #selector(tapStart), for: UIControl.Event.touchUpInside)
        ...
        self.view.addSubview(payer1.makeTitle(name: "\(name ?? "player5")さんの挑戦です"))
        makeMessage()
    ...
    @objc func resetData(){
        if (isFirst2!){
            ...
            if isUserNameSaved(){
                ...
                self.saveJustGetMiddleReultWithOthers(name: "player5", stroke: Double(CGFloat(stroke!)))
