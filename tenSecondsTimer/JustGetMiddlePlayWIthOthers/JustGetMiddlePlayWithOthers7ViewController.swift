//
//  JustGetMiddlePlayWithOthers7ViewController.swift
//  tenSecondsTimer
//
...
class JustGetMiddlePlayWithOthers7ViewController: UIViewController,UITextFieldDelegate{
...
//        self.startStop.addTarget(self, action: #selector(tapStart), for: UIControl.Event.touchUpInside)
    ...
    @objc func resetData(){
        if (isFirst2!){
            ...
            if isUserNameSaved(){
                self.saveJustGetMiddleReultWithOthers(name: "player7", stroke: Double(CGFloat(stroke!)))
