//
//  JustGetMiddlePlayWithOthers9ViewController.swift
//  tenSecondsTimer
//
...
class JustGetMiddlePlayWithOthers9ViewController: UIViewController,UITextFieldDelegate{
...
//        self.startStop.addTarget(self, action: #selector(tapStart), for: UIControl.Event.touchUpInside)
        ...
        self.view.addSubview(payer1.makeTitle(name: "\(name ?? "player9")さんの挑戦です"))
        makeMessagefirst()
...
//        改行するには、numberOfLines = 0にする必要があるらしい。なんか、tableViewでもnumberOfLinesを見たきたする
    ...
    @objc func resetData(){
        if (isFirst2!){
            ...
            if isUserNameSaved(){
                self.saveJustGetMiddleReultWithOthers(name: "player9", stroke: Double(CGFloat(stroke!)))
                ...
                default:
                viewController = storyBoard.instantiateViewController(identifier: "JustGetMiddlePlayWithOthers10ViewController")
                }
                ...
                self.saveJustGetMiddleReultWithOthers(name: name!, stroke: Double(CGFloat(stroke!)))
                    ...
                    default:
                        ...
                        alert.addAction(UIAlertAction(title: "入力完了", style: .default, handler: { [self] (action) in
                            let storyBoard = UIStoryboard(name: "Main", bundle: nil)
                    let viewController = storyBoard.instantiateViewController(identifier: "JustGetMiddlePlayWithOthers10ViewController") as! JustGetMiddlePlayWithOthers10ViewController
