//
//  JustGetMiddlePlayWithOthers10ViewController.swift
//  tenSecondsTimer
//
...
class JustGetMiddlePlayWithOthers10ViewController: UIViewController,UITextFieldDelegate{
...
//        self.startStop.addTarget(self, action: #selector(tapStart), for: UIControl.Event.touchUpInside)
        ...
        self.view.addSubview(payer1.makeTitle(name: "\(name ?? "player10")さんの挑戦です"))
        makeMessagefirst()
...
//        改行するには、numberOfLines = 0にする必要があるらしい。なんか、tableViewでもnumberOfLinesを見たきたする
    ...
    @objc func resetData(){
        if (isFirst2!){
            ...
            if isUserNameSaved(){
                self.saveJustGetMiddleReultWithOthers(name: "player10", stroke: Double(CGFloat(stroke!)))
